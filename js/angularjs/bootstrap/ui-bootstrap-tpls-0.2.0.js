  ...
  return {
    ...
    link:function (originalScope, element, attrs, modelCtrl) {
      ...
      element.bind('keydown', function (evt) {
          console.log(evt);
        if(attrs.preQuery && evt.target.value && (!evt.target.typekeyword || evt.keyCode == 229) ){
            scope.$eval(attrs.preQuery);            evt.target.typekeyword = evt.target.value;        }
        
        if(!evt.target.value){
            evt.target.typekeyword = null;
        }
